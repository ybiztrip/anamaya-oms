...
import ai.anamaya.service.oms.core.client.biztrip.mapper.request.BiztripHotelPropertyRateRequestMapper;
...
@Service
public class BiztripHotelPropertyRateService {

    private final WebClient biztripWebClient;
    private final BiztripAuthService authService;
    private final ObjectMapper mapper;

    private final BiztripHotelPropertyRateRequestMapper requestMapper =
        new BiztripHotelPropertyRateRequestMapper();

    private final BiztripHotelDiscoveryResponseMapper responseMapper =
        new BiztripHotelDiscoveryResponseMapper();

    public HotelDiscoveryResponse propertyRate(CallerContext callerContext, HotelPropertyRateRequest request) {
        try {
            String token = authService.getAccessToken(callerContext.companyId());

            BiztripHotelPropertyRateRequest biztripReq = requestMapper.map(request);
            logRequestAsCurl(biztripReq, token);

            String rawResponse = biztripWebClient.post()
                .uri("/hotel/v1/property-rate")
    ...
    private void logRequestAsCurl(BiztripHotelPropertyRateRequest request, String token) {
        try {
            String json = mapper.writerWithDefaultPrettyPrinter()
                .writeValueAsString(request);

            String curl = "curl -X POST \"" +
                "https://dev-affiliate.biztrip.id/hotel/v1/property-rate" +
                "\" \\\n" +
